...
theorem putnam_1991_a6
  ...
  have hfalse : False := by
    let S : Set (ℕ × (ℕ → ℕ)) := {a | nabsum 1 a ∧
      (∀ i : Fin (a.1 - 2), a.2 i > a.2 (i + 1) + a.2 (i + 2)) ∧
      1 < a.1 → a.2 (a.1 - 2) > a.2 (a.1 - 1)}
    have hSinf : S.Infinite := by
      refine Set.infinite_of_injective_forall_mem
        (s := S) (f := fun m : ℕ => (0, fun _ : ℕ => m)) ?_ ?_
      · intro m k hmk
        exact congrFun (congrArg Prod.snd hmk) 0
      · intro m
        change (nabsum 1 (0, fun _ : ℕ => m) ∧
          (∀ i : Fin (0 - 2),
            (fun _ : ℕ => m) i > (fun _ : ℕ => m) (i + 1) + (fun _ : ℕ => m) (i + 2)) ∧
          1 < (0 : ℕ) → (fun _ : ℕ => m) (0 - 2) > (fun _ : ℕ => m) (0 - 1))
        intro h
        exact False.elim ((Nat.not_lt_zero 1) h.2.2)
    have hencard : S.encard = ⊤ := hSinf.encard_eq
    have hA1 : (A 1 : ℕ∞) = S.encard := by
      simpa [S] using hA 1 (by norm_num)
    exact (ENat.coe_ne_top (A 1) (hA1.trans hencard)).elim
